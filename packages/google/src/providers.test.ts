  ...
  it("creates, updates, and sends Gmail drafts with a scoped access token", async () => {
    ...
    const provider = new GoogleGmailProvider(
      ...
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (request.url.endsWith("/drafts") && request.method === "POST")
          return response({ id: "draft-1" });
        if (request.url.endsWith("/drafts/draft-1") && request.method === "PUT")
          return response({ id: "draft-1" });
    ...
    expect(
      await provider.createDraft({ to: "sarah@example.com", subject: "Hello", body: "Body" }),
  ...
  it("reads free/busy and creates a Calendar meeting", async () => {
    ...
    const provider = new GoogleCalendarProvider(
      ...
      async (input: RequestInfo | URL, init?: RequestInit) => {
        const request = new Request(input, init);
        requests.push(request);
        if (request.url.endsWith("/freeBusy"))
          return response({ calendars: { primary: { busy: [{ start: "10:00", end: "11:00" }] } } });
    ...
    expect(
      requests.every((request) => request.headers.get("authorization") === "Bearer calendar-token"),
    ).toBe(true);
  });
});
